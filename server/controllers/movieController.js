  ...
  try {
    ...
    if (isExistedMovie.createdBy !== req.user.id) {
      return res.status(403).json({
        success: false,
        message: 'User is not allowed to remove this movie',
      });
    }
    ...
    return res.status(201).json({
      success: true,
      message: 'Movie removed successfully from the list',
    });
  } catch (error) {
...
export const updateMovie = async (req, res) => {
  try {
    ...
    if (isExistedMovie.createdBy !== req.user.id) {
      return res
        .status(403)
        .json({ success: false, message: 'Not allowed tu update this movie' });
    }
